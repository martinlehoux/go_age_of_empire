...
func NewFilledRectangleImage(size Point, color color.Color) *ebiten.Image {
	image := ebiten.NewImage(size.X, size.Y)
	image.Fill(color)
	return image
}

func NewStrokeRectangleImage(size Point, strokeWidth int, color color.Color) *ebiten.Image {
	image := ebiten.NewImage(size.X, size.Y)
